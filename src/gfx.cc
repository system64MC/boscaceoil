...
namespace bosca
{
	...
	struct Gfx
	{
		...
		void changescalemode(int t) {
			if (t == 0) {
				min_windowwidth = 768;
				min_windowheight = 540;
				stage.nativeWindow.minSize = Point(768 + windowboundsx, 540 + windowboundsy);
			}
			else {
				min_windowwidth = 1152;
				min_windowheight = 690;
				stage.nativeWindow.minSize = Point(1152 + windowboundsx, 690 + windowboundsy);
			}
		...
		int tutorialimagewidth(int t) {
			return images[t + 8].width;
		}

		int tutorialimageheight(int t) {
		...
		std::string lastchar(std::string s) {
			...
			if (j >= 1) {
					...
					if (control->musicbox[control->currentbox].start + i - 1 < control->drumkit[j].size) {
						if (control->musicbox[control->currentbox].start + i - 1 > -1) {
							...
							if (control->musicbox[control->currentbox].recordfilter == 1) {
								...
								print(this->screenwidthmid - (this->len("! ADVANCED FILTER EDITING ON !") / 2), screenheight - linesize - (i * linesize) + 1, "! ADVANCED FILTER EDITING ON !", 15, false);
							}
							else {
								fillrect(0, screenheight - linesize - (i * linesize), screenwidth, linesize, 12);
								print(this->screenwidthmid - (this->len("ADVANCED FILTER EDITING OFF") / 2), screenheight - linesize - (i * linesize) + 1, "ADVANCED FILTER EDITING OFF", 0);
							}
						}
					}
				}
				...
				for (i = 0; i < notesonscreen; i++) {
					if (control->musicbox[control->currentbox].start + i - 1 > -1) {
						...
						if (control->musicbox[control->currentbox].recordfilter == 1) {
							fillrect(0, screenheight - linesize - (i * linesize), screenwidth, linesize, 13);
							print(this->screenwidthmid - (this->len("! ADVANCED FILTER EDITING ON !") / 2), screenheight - linesize - (i * linesize) + 1, "! ADVANCED FILTER EDITING ON !", 15);
						}
						else {
							fillrect(0, screenheight - linesize - (i * linesize), screenwidth, linesize, 12);
							print(this->screenwidthmid - (this->len("ADVANCED FILTER EDITING OFF") / 2), screenheight - linesize - (i * linesize) + 1, "ADVANCED FILTER EDITING OFF", 0, true);
						}
					}
				}
			}
		...
		void drawpatterneditor_cursor() {
			...
			if (control->cursorx > -1 && control->cursory > -1) {
				if (control->musicbox[control->currentbox].start + control->cursory - 1 == -1) {
					if (control->doublesize) {
						drawbox(40 + (4 * control->boxsize), this->screenheight - linesize - (control->cursory * linesize), control->boxsize * 24, linesize, 0);
					}
					else {
						drawbox(40 + (2 * control->boxsize), this->screenheight - linesize - (control->cursory * linesize), control->boxsize * 12, linesize, 0);
					}
				}
				else {
					if (control->cursory == notesonscreen - 1) {
						//draw partial cursor
						drawpartialbox(40 + (control->cursorx * control->boxsize), this->screenheight - linesize - (control->cursory * linesize), control->boxsize * control->notelength, linesize, 0, pianorollposition + linesize);
					}
					else {
						drawbox(40 + (control->cursorx * control->boxsize), this->screenheight - linesize - (control->cursory * linesize), control->boxsize * control->notelength, linesize, 0);
						if (control->notelength > control->boxcount) {
							tempstring = std::string(control->notelength);
							print(40 + (control->cursorx * control->boxsize), this->screenheight - linesize - (control->cursory * linesize), tempstring, 0);
						}
					}
				}
			}
		...
		void drawmusicbox(int xp, int yp, int t, bool enabled = true, int forcezoom = -1) {
		...
		void drawpatternmanager_cursor()
		{
			//Draw the cursor
		...
		void initgfx() {
			...
			icons_rect = Rectangle(0, 0, 32, 32);
			trect = Rectangle{}; tpoint = Point();
		...
		void settrect(int x, int y, int w, int h) {
			trect.x = x;
			trect.y = y;
			trect.width = w;
			trect.height = h;
		...
		void addimage() {
			...
			t.copyPixels(buffer, Rectangle(0, 0, buffer.width, buffer.height), tl);
			images.push(t);
		}

		void drawimage(int t, int xp, int yp) {
			settpoint(xp, yp);
		...
		void makeiconarray() {
			for (int i = 0; i < 20; i++) {
				...
				Rectangle temprect = Rectangle(i * 32, 0, 32, 32);
		...
		void drawpartialbox(int x1, int y1, int w1, int h1, int col, int cutoff) {
			if (y1 > cutoff) {
		...
		void drawbox(int x1, int y1, int w1, int h1, int col) {
			settrect(x1, y1, w1, 2); backbuffer.fillRect(trect, RGB(pal[col].r, pal[col].g, pal[col].b));
			settrect(x1, y1 + h1 - 2, w1, 2); backbuffer.fillRect(trect, RGB(pal[col].r, pal[col].g, pal[col].b));
			settrect(x1, y1, 2, h1); backbuffer.fillRect(trect, RGB(pal[col].r, pal[col].g, pal[col].b));
			settrect(x1 + w1 - 2, y1, 2, h1); backbuffer.fillRect(trect, RGB(pal[col].r, pal[col].g, pal[col].b));
		...
		void fillrect(int x1, int y1, int w1, int h1, int t) {
			settrect(x1, y1, w1, h1);
			backbuffer.fillRect(trect, RGB(pal[t].r, pal[t].g, pal[t].b));
		}

		void drawbuffericon(int x, int y, int t) {
			settpoint(x, y);
		...
		void rprint(int x, int y, std::string t, int col, bool shadow = false) {
			x = x - len(t);
			print(x, y, t, col, false, shadow);
		}

		Dictionary cachedtextindex;
		std::vector<BitmapData> cachedtext = new std::vector<BitmapData>;
		std::vector<Rectangle> cachedrect = new std::vector<Rectangle>;
		int cacheindex;
		std::string cachelabel;

		void print(int x, int y, std::string t, int col, bool cen = false, bool shadow = false) {
			if (shadow) {
			...
			if (cachedtextindex[cachelabel] == null) {
				...
				cachedrect.push(Rectangle(0, 0, len(t), 22));
		...
		void printoncache(int x, int y, std::string t, int col, bool cen = false, bool shadow = false) {
			y -= 3;
		...
		void normalprint(int x, int y, std::string t, int col, bool cen = false, bool shadow = false) {
			y -= 3;
		...
		void rbigprint(int x, int y, std::string t, int r, int g, int b, bool cen = false, float sc = 2) {
			x = x - len(t, sc);
			bigprint(x, y, t, r, g, b, cen, sc);
		}

		void bigprint(int x, int y, std::string t, int r, int g, int b, bool cen = false, float sc = 2) {
		...
		float RGB(float red, float green, float blue) {
		...
		// std::vector<BitmapData> icons = new std::vector<BitmapData>;
		ColorTransform ct;
		Rectangle icons_rect;
		Point tl = Point(0, 0);
		// std::vector<BitmapData> images = new std::vector<BitmapData>;
		Rectangle trect; Point tpoint; BitmapData tbuffer;
		int i; int j; int k; int l; int mbi; int mbj;
		std::string tempstring;

		int screenwidth; int screenheight;
		int screenwidthmid; int screenheightmid;
		int screenviewwidth; int screenviewheight;
		int linesize; int patternheight; int patternwidth;
		int temppatternwidth;
		int patternmanagerx;
		int linespacing;
		int patterneditorheight;
		int buttonheight;
		int pianorollposition;
		int notesonscreen;

		int temp; int temp2; int temp3;
		uint alphamult;
		std::string stemp;
		BitmapData buffer;
		int temppal;

		int zoom, float zoomoffset;

		BitmapData tempicon;
		//Actual backgrounds
		BitmapData drawto;
		BitmapData backbuffer;
		BitmapData backbuffercache;
		int updatebackground;
		BitmapData screenbuffer;
		Bitmap screen;
		//Tempshape
		...
		int min_windowwidth; int min_windowheight;
		int windowboundsx; int windowboundsy;
		int scalemode;

		int boscaframerate = -1;

		int arrangementscrollleft = 0;
		int arrangementscrollright = 0;
	}
}