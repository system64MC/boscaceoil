namespace bosca {
	struct guiclass {
		...
		void addline(std::string line, std::string high = "", std::string high2 = "") {
			if(line != "") {
		...
		void addwindow(int x, int y, int w, int h, std::string text) {
			if(helpwindow != "nothing"){
				addguipart(x, y, w, h, windowtext, "window", "window");
			}
		}
		
		void addtutorialimage(int x, int y, int img, bool towindow = false) {
			addguipart(x, y, 0, 0, "", "", "tutorialimage", img);
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addbutton(int x, int y, int w, std::string text, std::string action, int textoffset = 0, bool towindow = false) {
			addguipart(x, y, w, gfx->buttonheight, text, action, "normal", textoffset);
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addlogo(int x, int y, bool towindow = false) {
			addguipart(x, y, 356, 44, "BOSCA CEOIL", "logo", "logo");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addtextlabel(int x, int y, std::string text, int col = 2, bool towindow = false) {
			addguipart(x, y, col, 0, text, "", "textlabel");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addcentertextlabel(int x, int y, int w, std::string text, int col = 2, bool towindow = false) {
			addguipart(x + ((w / 2) - (gfx->len(text) / 2)), y, col, 0, text, "", "textlabel");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addrighttextlabel(int x, int y, std::string text, int col = 2, bool towindow = false) {
			addguipart(x, y, col, 0, text, "", "righttextlabel");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addblackout(bool towindow = false) {
			addguipart(0, 0, 0, 0, "", "", "blackout", 12);
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addrect(int x, int y, int w, int h, int col = 1, std::string action = "", bool towindow = false) {
			addguipart(x, y, w, h, "", action, "fillrect", col);
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addhighlight(int x, int y, int w, int h, int col = 1, std::string action = "", bool towindow = false) {
			highlightflash = 30;
			addguipart(x, y, w, h, "", action, "highlight", col);
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addleftarrow(int x, int y, std::string action, bool towindow = false) {
			addguipart(x, y, 16, 16, "", action, "leftarrow");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addrightarrow(int x, int y, std::string action, bool towindow = false) {
			addguipart(x, y, 16, 16, "", action, "rightarrow");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addplayarrow(int x, int y, std::string action, bool towindow = false) {
			addguipart(x, y, 16, 16, "", action, "playarrow");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addpausebutton(int x, int y, std::string action, bool towindow = false) {
			addguipart(x, y, 16, 16, "", action, "pause");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addstopbutton(int x, int y, std::string action, bool towindow = false) {
			addguipart(x, y, 16, 16, "", action, "stop");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addplusbutton(int x, int y, std::string action, bool towindow = false) {
			addguipart(x, y, 16, 16, "", action, "plus");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addminusbutton(int x, int y, std::string action, bool towindow = false) {
			addguipart(x, y, 16, 16, "", action, "minus");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void adddownarrow(int x, int y, std::string action, bool towindow = false) {
			addguipart(x, y, 16, 16, "", action, "downarrow");
			if (towindow) button[lastbutton].onwindow = true;
		}
			
		void adduparrow(int x, int y, std::string action, bool towindow = false) {
			addguipart(x, y, 16, 16, "", action, "uparrow");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addscrollupbutton(int x, int y, int w, std::string action, bool towindow = false) {
			addguipart(x, y, w, 21, "", action, "scrollup");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addscrolldownbutton(int x, int y, int w,std::string action, bool towindow = false) {
			addguipart(x, y, w, 21, "", action, "scrolldown");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addvariable(int x, int y, std::string variable, int col = 0, bool towindow = false) {
			addguipart(x, y, col, 0, "", variable, "variable");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addhorizontalslider(int x, int y, int w, std::string variable, bool towindow = false) {
			addguipart(x, y, w, 26, "", variable, "horizontalslider");
			if (towindow) button[lastbutton].onwindow = true;
		}
		
		void addcontrol(int x, int y, std::string type) {
		...
		void addguipart(int x, int y, int w, int h, std::string contents, std::string act = "", std::string sty = "normal", int toffset = 0) {
		...
		bool buttonexists(std::string t) {
		...
		void selectbutton(std::string t) {
		...
		int findbuttonbyaction(std::string t) {
			for (int i = 0; i < numbuttons; i++) {
				if (button[i].active) {
					if (button[i].action == t) {
						return i;
		...
		int windowddx, int windowddy;
		int windowdx, int windowdy;
		...
		std::string helpwindow;
		
		std::string helpcondition_check;
		std::string helpcondition_set;
